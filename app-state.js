...
const remember=item=>{if(!item?.key)return;const next=[{...item,seenAt:Date.now()},...recent().filter(x=>x.key!==item.key)].slice(0,10);write(KEYS.recent,next)};
const notes=()=>read(KEYS.notes,{});
const getNote=key=>notes()[key]||'';
const setNote=(key,text)=>write(KEYS.notes,{...notes(),[key]:String(text||'')});
...
const settings=()=>read(KEYS.settings,{theme:'dark',compact:false,haptics:true});
const patchSettings=patch=>write(KEYS.settings,{...settings(),...patch});
const exportWorkspace=()=>({version:1,app:'ToneForge Workshop',exportedAt:new Date().toISOString(),favorites:favorites(),recent:recent(),inventory:inventory(),notes:notes(),settings:settings(),buildStatus:Object.fromEntries(Object.keys(localStorage).filter(k=>k.startsWith('gbe-build-status-v3:')||k.startsWith('gbe-marker-v2:')).map(k=>[k,localStorage.getItem(k)]))});
const importWorkspace=data=>{if(!data||typeof data!=='object')throw new Error('Ungültige Workspace-Datei');if(Array.isArray(data.favorites))write(KEYS.favorites,data.favorites);if(Array.isArray(data.recent))write(KEYS.recent,data.recent);if(data.inventory&&typeof data.inventory==='object')write(KEYS.inventory,data.inventory);if(data.notes&&typeof data.notes==='object')write(KEYS.notes,data.notes);if(data.settings&&typeof data.settings==='object')write(KEYS.settings,data.settings);if(data.buildStatus&&typeof data.buildStatus==='object')Object.entries(data.buildStatus).forEach(([k,v])=>{if((k.startsWith('gbe-build-status-v3:')||k.startsWith('gbe-marker-v2:'))&&typeof v==='string')localStorage.setItem(k,v)});return true};
...
const share=async({title,text,url=location.href}={})=>{if(navigator.share){try{await navigator.share({title,text,url});return true}catch(e){if(e?.name==='AbortError')return false}}try{await navigator.clipboard.writeText([title,text,url].filter(Boolean).join('\n'));return true}catch{return false}};
const haptic=(pattern=12)=>{if(settings().haptics&&navigator.vibrate)navigator.vibrate(pattern)};
...
window.ToneForgeState={KEYS,read,write,favorites,isFavorite,toggleFavorite,recent,remember,getNote,setNote,inventory,setInventory,settings,patchSettings,exportWorkspace,importWorkspace,download,share,haptic,projectProgress};
})();
